...
const myBot = ['u446015c6dce1ba353408e4f01477abc0','uf4bdda7a9a6506a04ba6acf087453492'];
...
\n\n# Gunakan bot dengan bijak ^_^";
    ...
    poll(operation) {
		...
		if(operation.type == 5 && this.stateStatus.salam == 1) {//someone adding me..
            let halo = new Message();
			halo.to = operation.param1;
			halo.text = "Creator:  line://ti/p/nong1426 (✎โหน่ง รับติดบอทsiri)";
			this._client.sendMessage(0, halo);
        }
...
Link Download: "+idU.id+"\n";
        ...
        if(txt == '!halo') {
            ...
            this._sendMessage(seq, 'บอทโหน่ง By  '+displayName);
        }
		...
		if(txt == '0101') {//Jangan dicoba (gk ada efek)
            ...
            for (var i = 0; i < listMember.length; i++) {
                if(listMember[i].mid==param){
					let namanya = listMember[i].dn;
					seq.text = ' สวัสดีครับ @'+namanya+' 􀄃􀉘pointing right􏿿ลูกค้าใหม่อ่านรายละเอียดในโน้ตนะครับ ส่วนคำเล่นบอทดูในอัลบั้มเลยครับ􀄃􀄰scissors􏿿􀄃􀄰scissors􏿿';
					let midnya = listMember[i].mid;
